                        ...
                        @endphp
                            ...
                            <td>
                                <div class="font-weight-bold text-primary">
                                    <a href="{{ route('admin.sourcing.show', $item->folder->uuid)}} ">{{ $item->folder->num_bl ?? '--'}}</a>
                                </div>
                            </td>
                            <td class="text-danger"> {{ $date_alerte ?? '--'}} </td>
                            <td>{{ Carbon\Carbon::parse($item->folder->date_arriver)->format('d/m/y') ?? '--'}}</td>
                            <td>{{ Carbon\Carbon::parse($item->folder->date_depart)->format('d/m/y') ?? '--'}}</td>
                            <td>{{ $item->folder->products->count() ?? '--'}}</td>
                            @foreach ($docs as $doc )
                            <td>
                                <div class="form-check form-switch  form-check-success" clickedUrl="{{ route('admin.updateStatusFolder') }}" value="{{$doc->uuid}}" name="folderCheck" sourcing="{{$item->folder->uuid}}">
                                    <input class="form-check-input folderCheck" unchecked clickedUrl="{{ route('admin.updateStatusFolder') }}" value="{{$doc->uuid}}" name="folderCheck" sourcing="{{$item->folder->uuid}}" type="checkbox"
                                    @if (isfolderCheck($item->folder->uuid,$doc->uuid))
                                        {{"checked disabled"}} 
                                    @endif
                            ...
                            <td>
                                @if ($item->backup)
                                    <span class="text-info size_12">
                                        @if($item->backup)
                                            {{$item->backup->name.' '.$item->backup->lastname}}
                                        @else
                                            Aucun Backup
                                        @endif
                                    </span>
